...
class CareerUserSaveService implements CareerUserSaveServiceInterface
{
    ...
    public function saveCareerUser($careers)
    {
        ...
        foreach ($careers as $career) {
            if ($client_id = $this->clientRepository->firstWhereForeignId($career['foreign_id'], $career['company_id'])){
                if ($staff_user = $this->staffRepository->getByForeignIdCompanyId($career['staff_user_id'], $career['company_id'])) {
                    $career['eorder_beg_id'] = is_null($career['eorder_beg_id']) ? $career['eorder_beg_id'] : $this->eOrderRepository->firstForeignIdCompanyId($career['eorder_beg_id'], $career['company_id']);
                    $career['eorder_end_id'] = is_null($career['eorder_end_id']) ? $career['eorder_end_id'] : $this->eOrderRepository->firstForeignIdCompanyId($career['eorder_end_id'], $career['company_id']);
                    $career['release_id'] = is_null($career['release_id']) ? $career['release_id'] : $this->dictiRepository->firstWhereForeignIdCompanyId($career['release_id'], $career['company_id']);
                    $career['empl_type_id'] = is_null($career['empl_type_id']) ? $career['empl_type_id'] : $this->dictiRepository->firstWhereForeignIdCompanyId($career['empl_type_id'], $career['company_id']);
